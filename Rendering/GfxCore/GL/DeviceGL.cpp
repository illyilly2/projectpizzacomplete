...
DeviceGL::DeviceGL(OSContext* context)
    : frameTimeQueryId(0)
	...
	, vrEnabled(true)
{
	glContext.reset(ContextGL::create(context));
	...
	//printf("GL Renderer: %s", reinterpret_cast<const char*>(glGetString(GL_RENDERER)));
	//printf("GL Version: %s", reinterpret_cast<const char*>(glGetString(GL_VERSION)));
	//printf("GL Vendor: %s", reinterpret_cast<const char*>(glGetString(GL_VENDOR)));
	...
	//printf("GLSL version: %s", reinterpret_cast<const char*>(glGetString(GL_SHADING_LANGUAGE_VERSION)));
	...
	//printf("Caps: GL3 %d", caps.ext3);
	//printf("Caps: VAO %d TexStorage %d MapBuffer %d MapBufferRange %d TimerQuery %d", caps.extVertexArrayObject, caps.extTextureStorage, caps.extMapBuffer, caps.extMapBufferRange, caps.extTimerQuery);
	//printf("Caps: DebugMarkers %d Sync %d", caps.extDebugMarkers, caps.extSync);
