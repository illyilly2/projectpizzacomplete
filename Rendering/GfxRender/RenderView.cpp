...
FASTFLAGVARIABLE(RenderLowLatencyLoop, true)
    ...
    , frameDataCallback(FrameDataCallback())
{
    ...
    for (unsigned i = 0; i < validGraphicsModes; ++i)
    {
        ...
        {
            device.reset(Device::create(gGraphicsModesTranslation[i].api, context));
...
{
    if (auto deviceGL = dynamic_cast<DeviceGL*>(device.get()))
    {
        if (auto framebuffer = dynamic_cast<FramebufferGL*>(deviceGL->getMainFramebuffer()))
        {
            framebuffer->setId(deviceGL->getMainFrameBufferId());
        }
    }
}
