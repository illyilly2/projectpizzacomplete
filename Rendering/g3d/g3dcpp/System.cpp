...
#    ifdef RBX_PLATFORM_DURANGO
    ...
    m_cpuArch = "WASM (Emscripten)";
    m_operatingSystem = "None";
...
void System::cpuid(CPUIDFunction func, uint32& areg, uint32& breg, uint32& creg, uint32& dreg) {
    ...
    areg = static_cast<uint32>(cpuInfo[0]);  // eax
    breg = static_cast<uint32>(cpuInfo[1]);  // ebx
    creg = static_cast<uint32>(cpuInfo[2]);  // ecx
